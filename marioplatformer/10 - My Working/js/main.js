...
gameScene.create = function() {
  ...
  // show me where I am clicking on the screen
...
gameScene.update = function () {
  ...
  if (this.cursors.left.isDown && !this.cursors.right.isDown) {
    ...
    // make the player animation stop
    this.player.anims.stop('walking');

    // set default player frame
    if(onGround)
...
// sets up all the elements of the game level
gameScene.setupLevel = function(){
  ...
  for(let i = 0; i < this.levelData.platforms.length; i++) {
    ...
    // add to the group for collision detection
  ...
  // constrain the player to the game bounds
...
gameScene.setupSpawner = function(){
  ...
  let spawningEvent = this.time.addEvent({
    ...
    callback: function() {
      ...
      // add overlap detection for barrels and player
      //this.physics.add.overlap(this.player, barrel, this.restartGame, null, this);
    }
  });
};
